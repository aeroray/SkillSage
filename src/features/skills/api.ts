...
import type { InstallResult, InstalledSkillsList, UpdateCheckList, InstalledSkill } from "./types";
...
export function adjustDistribution(skillId: string, agents: string[]) {
  return invokeCommand<InstalledSkill>("adjust_distribution", { skillId, agents });
}

export function distributeSkills(skillIds: string[], agents: string[]) {
  return invokeCommand<InstalledSkillsList>("distribute_skills", { skillIds, agents });
}
